export const heroContent = {
  eyebrow: "Homestay Coco Island",
  title: "Trải nghiệm lưu trú chính chủ tại Cồn Phụng Bến Tre",
  description:
    "Khu nghỉ dưỡng Homestay Coco Island bao gồm 30 phòng (diện tích từ 12-20 mét vuông). Những căn nhà gỗ nằm sát sông với thảm lục bình xanh mát và cây cầu dừa dài 300 m đưa bạn vào trạng thái thư giãn thuần miền Tây.",
  primaryCta: {
    label: "Đặt ngay",
    href: "#booking",
  },
  secondaryCta: {
    label: "Xem phòng",
    href: "#rooms",
  },
  stats: [
    { label: "Phòng nghỉ", value: "30+" },
    { label: "Cầu dừa", value: "300m" },
    { label: "Giá chỉ từ", value: "800.000₫" },
  ],
  heroImage:
    "/cocoisland/2023/01/coco-island-con-phung-ben-tre3.jpg",
  video: {
    url: "https://youtube.com/shorts/-NK90Tub16E?feature=share",
    poster:
      "/cocoisland/2023/01/coco-island-con-phung-ben-tre12.jpg",
  },
};

export const stayPerks = {
  heading: "Chế độ ưu đãi phòng nghỉ & ăn sáng",
  items: [
    "2 chai nước suối mỗi ngày",
    "1 dĩa trái cây chào mừng",
    "Cà phê và trà miễn phí",
    "2 trái dừa tươi",
    "Bữa sáng đặc sản hủ tiếu",
  ],
};

export const experiences = [
  {
    title: "Cầu dừa dài nhất miền Tây",
    description:
      "Cây cầu dừa dài 300 m nối các căn bungalow dọc bờ sông tạo nên điểm check-in đặc trưng của Coco Island.",
  },
  {
    title: "Không gian xanh mát",
    description:
      "Bao quanh từng phòng là thảm lục bình và hàng bần xanh, mang đến không khí trong lành và yên bình.",
  },
  {
    title: "Dịch vụ bản địa",
    description:
      "Đội ngũ chính chủ tại Cồn Phụng chăm sóc chu đáo, các món ăn truyền thống miền sông nước luôn sẵn sàng phục vụ.",
  },
];

export const restaurantSection = {
  eyebrow: "Eat & Drink",
  title: "Nhà hàng - Ẩm thực",
  description:
    "Thưởng thức đặc sản miền Tây ngay tại homestay với thực đơn phong phú: cá tai tượng chiên xù, lẩu mắm, bánh xèo, chè dừa… Đội ngũ phục vụ tận tâm giúp bữa ăn gia đình hay đoàn khách thêm trọn vẹn.",
  image:
    "/cocoisland/2023/01/coco-island-con-phung-ben-tre20.jpg",
};

export const discoverySection = {
  eyebrow: "Chương trình",
  title: "Khám phá vùng đất Tứ Linh: Long – Lân – Quy – Phụng",
  description:
    "Lịch trình cano / đò đưa bạn len lỏi qua rạch nhỏ, ghé thăm lò kẹo dừa, nhà cổ hơn 150 năm tuổi, vườn mật ong, thưởng thức đờn ca tài tử và trải nghiệm chèo xuồng giữa rừng dừa nước.",
  highlights: [
    "Tham quan cồn Long – Lân – Quy – Phụng",
    "Nghe đờn ca tài tử, uống trà mật ong",
    "Dừng chân thưởng thức trái cây miệt vườn",
    "Trải nghiệm làm kẹo dừa truyền thống",
  ],
  image:
    "/cocoisland/2021/06/coco-island-con-phung-ben-tre13-510x510.jpg",
};

export const testimonials = [
  {
    author: "Nguyễn Minh Hữu",
    role: "Khách đoàn công ty",
    quote:
      "Coco Island rất đẹp, không khí trong lành. Nhân viên thân thiện, món ăn ngon. Đoàn của mình ai cũng khen hết lời.",
  },
  {
    author: "Lan Anh",
    role: "Khách gia đình",
    quote:
      "Phòng sạch sẽ, view sông rất chill. Trẻ con thích thú với cầu dừa và các hoạt động dân gian. Chắc chắn sẽ quay lại.",
  },
];

export const services = [
  {
    title: "Đón tiếp tại bến tàu",
    description:
      "Hướng dẫn viên hỗ trợ đưa khách lên ghe, đảm bảo an toàn suốt chuyến tham quan.",
  },
  {
    title: "Tour trải nghiệm",
    description:
      "Combo cano, hát đờn ca tài tử, tham quan lò kẹo dừa và các điểm check-in đặc sắc.",
  },
  {
    title: "Giải trí & thư giãn",
    description:
      "Đạp xe, câu cá, chèo xuồng, tắm sông, xem biểu diễn nghệ thuật dân gian.",
  },
  {
    title: "Sự kiện & hội nghị",
    description:
      "Không gian tổ chức team building, gala dinner, hội thảo doanh nghiệp giữa thiên nhiên.",
  },
];

export const contactInfo = {
  phone: "+84 918 267 715",
  email: "conphungtourist87@gmail.com",
  address: "89P9+GRW, Tân Thạch, Châu Thành, Bến Tre",
  mapUrl: "https://goo.gl/maps/67bzTFmzr9kxWmY17",
  hotlineLabel: "Liên hệ đặt phòng",
};

export const newsletterContent = {
  title: "Nhận khuyến mãi từ chúng tôi",
  description:
    "Đăng ký để nhận tin ưu đãi mới nhất về phòng nghỉ, combo tour và sự kiện tại Coco Island.",
};

export const defaultRooms = [
  {
    id: "default-honey-moon",
    title: "HOMESTAY HONEY MOON ROOM",
    slug: "homestay-honey-moon-room",
    summary:
      "Chúng tôi là khu nghỉ dưỡng Coco Island bao gồm 30 phòng/1 Homestay HONEY MOON (diện tích phòng 20 mét vuông). Căn phòng được thiết kế dành riêng cho các cặp đôi muốn tận hưởng không gian riêng tư bên bờ sông.",
    thumbnail:
      "/cocoisland/2021/06/coco-island-con-phung-ben-tre8-770x550.jpg",
    amenities: ["1 giường lớn", "1 phòng tắm", "Ban công nhìn sông"],
    maxGuests: 2,
    basePrice: "1.500.000₫ / đêm",
  },
  {
    id: "default-vip-room",
    title: "HOMESTAY VIP ROOM (1 giường)",
    slug: "homestay-vip-room",
    summary:
      "Chúng tôi là khu nghỉ dưỡng Coco Island bao gồm 30 phòng/5 Homestay VIP ROOM (mỗi phòng 20 mét vuông). Không gian rộng rãi, nội thất gỗ cao cấp phù hợp cặp đôi hoặc khách nghỉ dưỡng.",
    thumbnail:
      "/cocoisland/2021/06/coco-island-con-phung-ben-tre4-770x550.jpg",
    amenities: ["1 giường lớn", "1 phòng tắm", "Ăn sáng miễn phí"],
    maxGuests: 2,
    basePrice: "1.200.000₫ / đêm",
  },
  {
    id: "default-family-room",
    title: "HOMESTAY FAMILY ROOM (2 giường)",
    slug: "homestay-family-room",
    summary:
      "Chúng tôi là khu nghỉ dưỡng Coco Island bao gồm 30 phòng/5 Homestay FAMILY ROOM (mỗi phòng 20 mét vuông). Lựa chọn lý tưởng cho gia đình nhỏ với không gian ấm cúng và tiện nghi đầy đủ.",
    thumbnail:
      "/cocoisland/2022/12/coco-island-con-phung-ben-tre-770x550.jpg",
    amenities: ["2 giường đôi", "1 phòng tắm", "Khu vực tiếp khách"],
    maxGuests: 4,
    basePrice: "1.600.000₫ / đêm",
  },
  {
    id: "default-double-room",
    title: "HOMESTAY DOUBLE ROOM (1 giường)",
    slug: "homestay-double-room",
    summary:
      "Chúng tôi là khu nghỉ dưỡng Coco Island bao gồm 30 phòng/19 Homestay DOUBLE ROOM (mỗi phòng 12 mét vuông). Mức giá dễ chịu, phù hợp nhóm bạn, cặp đôi yêu thích trải nghiệm thiên nhiên.",
    thumbnail:
      "/cocoisland/2021/06/coco-island-con-phung-ben-tre11-770x550.jpg",
    amenities: ["1 giường đôi", "1 phòng tắm", "Wi-Fi miễn phí"],
    maxGuests: 2,
    basePrice: "800.000₫ / đêm",
  },
];

type RoomDetail = {
  slug: string;
  title: string;
  subtitle: string;
  description: string[];
  heroImage: string;
  gallery: string[];
  amenities: string[];
  inclusions: string[];
  basePrice?: string;
  size?: string;
  maxGuests?: number;
  bedInfo?: string;
  policies?: string[];
};

export const defaultRoomDetails: Record<string, RoomDetail> = {
  "homestay-honey-moon-room": {
    slug: "homestay-honey-moon-room",
    title: "Homestay Honey Moon Room",
    subtitle: "Diện tích 20m² · 1 giường đôi lớn · 2 khách",
    description: [
      "Chúng tôi là khu nghỉ dưỡng Coco Island bao gồm 30 phòng/1 Homestay Honey Moon (diện tích phòng 20 mét vuông). Căn phòng được thiết kế dành riêng cho các cặp đôi muốn tận hưởng không gian riêng tư bên bờ sông.",
      "Nhà gỗ ấm áp, ban công hướng sông và những dịch vụ chuẩn chính chủ giúp kỳ nghỉ của bạn thêm trọn vẹn.",
    ],
    heroImage:
      "/cocoisland/2021/06/coco-island-con-phung-ben-tre8-770x550.jpg",
    gallery: [
      "/cocoisland/2021/06/coco-island-con-phung-ben-tre8-770x550.jpg",
      "/cocoisland/2022/12/coco-island-con-phung-ben-tre-770x550.jpg",
      "/cocoisland/2021/06/coco-island-con-phung-ben-tre11-770x550.jpg",
    ],
    amenities: [
      "Máy lạnh, TV màn hình phẳng",
      "Két an toàn & minibar",
      "Phòng tắm riêng với nước nóng",
      "Ban công nhìn sông",
      "Wi-Fi tốc độ cao",
    ],
    inclusions: [
      "Bữa sáng hủ tiếu chính gốc",
      "Trái cây miệt vườn chào mừng",
      "2 trái dừa tươi và trà, cà phê tại phòng",
    ],
    basePrice: "1.500.000₫ / đêm",
    size: "20m²",
    maxGuests: 2,
    bedInfo: "1 giường đôi lớn",
    policies: [
      "Nhận phòng 14:00 - Trả phòng 12:00",
      "Hủy miễn phí trước 3 ngày",
      "Không hút thuốc trong phòng",
    ],
  },
  "homestay-vip-room": {
    slug: "homestay-vip-room",
    title: "Homestay VIP Room (1 giường)",
    subtitle: "Diện tích 20m² · 1 giường Queen · 2 khách",
    description: [
      "Chúng tôi là khu nghỉ dưỡng Coco Island bao gồm 30 phòng/5 Homestay VIP Room (mỗi phòng 20 mét vuông). Không gian rộng rãi, nội thất gỗ cao cấp phù hợp cặp đôi hoặc khách nghỉ dưỡng.",
      "Tất cả phòng đều nằm cạnh bờ sông với tầm nhìn bao quát, thích hợp để tận hưởng hoàng hôn trên Cồn Phụng.",
    ],
    heroImage:
      "/cocoisland/2021/06/coco-island-con-phung-ben-tre4-770x550.jpg",
    gallery: [
      "/cocoisland/2021/06/coco-island-con-phung-ben-tre4-770x550.jpg",
      "/cocoisland/2022/12/coco-island-con-phung-ben-tre-770x550.jpg",
      "/cocoisland/2021/06/vip-510x510.jpg",
    ],
    amenities: [
      "Máy lạnh, TV, minibar",
      "Bàn làm việc & ghế sofa",
      "Buồng tắm đứng với nước nóng",
      "Ban công riêng nhìn ra sông",
      "Wifi miễn phí",
    ],
    inclusions: [
      "Bữa sáng cho 2 khách",
      "Trái cây miệt vườn chào mừng",
      "2 trái dừa tươi & trà, cà phê",
    ],
    basePrice: "1.200.000₫ / đêm",
    size: "20m²",
    maxGuests: 2,
    bedInfo: "1 giường lớn",
    policies: [
      "Nhận phòng 14:00 - Trả phòng 12:00",
      "Hủy miễn phí trước 5 ngày",
      "Không mang thú cưng vào phòng",
    ],
  },
  "homestay-family-room": {
    slug: "homestay-family-room",
    title: "Homestay Family Room (2 giường)",
    subtitle: "Diện tích 20m² · 2 giường đôi · 4 khách",
    description: [
      "Chúng tôi là khu nghỉ dưỡng Coco Island bao gồm 30 phòng/5 Homestay Family Room (mỗi phòng 20 mét vuông). Lựa chọn lý tưởng cho gia đình nhỏ với không gian ấm cúng và tiện nghi đầy đủ.",
      "Phòng có ban công rộng và khu vực tiếp khách, rất phù hợp cho trẻ em vui chơi và gia đình quây quần.",
    ],
    heroImage:
      "/cocoisland/2022/12/coco-island-con-phung-ben-tre-770x550.jpg",
    gallery: [
      "/cocoisland/2022/12/coco-island-con-phung-ben-tre-770x550.jpg",
      "/cocoisland/2021/06/homestay-FAMILY-ROOM-coco-island-con-phung-ben-tre--510x510.jpg",
      "/cocoisland/2021/06/coco-island-con-phung-ben-tre12-770x550.jpg",
    ],
    amenities: [
      "2 giường đôi, khu vực tiếp khách",
      "Máy lạnh, TV, tủ lạnh mini",
      "Phòng tắm riêng với nước nóng",
      "Ban công rộng nhìn sông",
      "Wi-Fi miễn phí",
    ],
    inclusions: [
      "Bữa sáng cho 4 khách",
      "Trái cây miệt vườn và 2 trái dừa",
      "Trà & cà phê miễn phí",
    ],
    basePrice: "1.600.000₫ / đêm",
    size: "20m²",
    maxGuests: 4,
    bedInfo: "2 giường đôi",
    policies: [
      "Nhận phòng 14:00 - Trả phòng 12:00",
      "Phụ thu trẻ em theo chính sách homestay",
      "Không hút thuốc trong phòng",
    ],
  },
  "homestay-double-room": {
    slug: "homestay-double-room",
    title: "Homestay Double Room (1 giường)",
    subtitle: "Diện tích 12m² · 1 giường đôi · 2 khách",
    description: [
      "Chúng tôi là khu nghỉ dưỡng Coco Island bao gồm 30 phòng/19 Homestay Double Room (mỗi phòng 12 mét vuông). Mức giá dễ chịu, phù hợp nhóm bạn, cặp đôi yêu thích trải nghiệm thiên nhiên.",
      "Không gian vừa đủ với đầy đủ tiện nghi cơ bản, mở cửa bước ra là cầu dừa và khung cảnh sông nước miền Tây.",
    ],
    heroImage:
      "/cocoisland/2021/06/coco-island-con-phung-ben-tre11-770x550.jpg",
    gallery: [
      "/cocoisland/2021/06/coco-island-con-phung-ben-tre11-770x550.jpg",
      "/cocoisland/2021/06/coco-island-con-phung-ben-tre9-510x510.jpg",
      "/cocoisland/2021/06/coco-island-con-phung-ben-tre5-510x510.jpg",
    ],
    amenities: [
      "Giường đôi, máy lạnh, TV",
      "Phòng tắm riêng với nước nóng",
      "Wifi miễn phí",
      "Tiện nghi pha trà & cà phê",
    ],
    inclusions: [
      "Bữa sáng 2 khách",
      "Trái cây chào mừng",
      "Nước uống trong phòng",
    ],
    basePrice: "800.000₫ / đêm",
    size: "12m²",
    maxGuests: 2,
    bedInfo: "1 giường đôi",
    policies: [
      "Nhận phòng 14:00 - Trả phòng 12:00",
      "Không mang thức ăn có mùi mạnh vào phòng",
      "Giữ yên tĩnh sau 22:00",
    ],
  },
};

export type { RoomDetail };
