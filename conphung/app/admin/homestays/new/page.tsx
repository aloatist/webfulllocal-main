...
const buildInitialFormState = (initial?: HomestayEditorData | null): HomestayFormState => ({
  ...
  galleryImageUrls: initial?.media?.map((item) => item.url) ?? [],
...
const buildInitialAvailability = (): AvailabilityBlock[] => [];
    ...
    const timeout = window.setTimeout(async () => {
      try {
        ...
        const results = Array.isArray(data?.data)
          ...
          ? data
          : [];
        const conflict = results.some((item: any) => item?.slug === form.slug);
        if (conflict) {
  ...
  const handleSubmit = async (targetStatus: 'DRAFT' | 'PUBLISHED') => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(json.error || 'Không thể tạo homestay');
      }
      ...
      setError(submitError instanceof Error ? submitError.message : 'Lỗi tạo dữ liệu');
    } finally {
      setSaving(false);
    }
  };
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div className="space-y-3">
            <div className="space-y-1">
              <p className="text-xs uppercase tracking-wide text-muted-foreground">Homestay · Tạo mới</p>
              <h1 className="text-2xl font-semibold">Thiết lập homestay mới</h1>
        ...
        >
              ...
              <div className="md:col-span-2 space-y-3">
                ...
                <div className="flex flex-wrap gap-2">
                  {form.galleryImageUrls.map((imageUrl) => (
                    <span
                      key={imageUrl}
                      className="group relative flex h-20 w-28 items-center justify-center overflow-hidden rounded border border-border"
                    >
                      {/* eslint-disable-next-line @next/next/no-img-element */}
                      <img src={imageUrl} alt="" className="h-full w-full object-cover" />
                      <button
                        type="button"
                        className="absolute right-1 top-1 rounded-full bg-black/50 px-2 text-xs text-white opacity-0 transition group-hover:opacity-100"
          ...
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-end">
            ...
            <div className="flex gap-2">
              ...
              >
                {saving && form.status === 'DRAFT' ? 'Đang lưu...' : 'Lưu nháp'}
              </button>
              <button
                type="button"
              ...
              >
                {saving && form.status === 'PUBLISHED' ? 'Đang xuất bản...' : 'Xuất bản'}
              </button>
            </div>
          </div>
        </form>
...
export default function NewHomestayPage() {
  return <HomestayEditor mode="create" redirectTo="/admin/homestays" />;
}
